...
//클래스, 구조체, 익스텐션, 열거형 ...
//클래스는 단일 상속만 가능하지만, 포로토콜은 채택 갯수에 제한이 없다.
//@objc optional > 선택적 요청 (Optional Requirement)
//프로토콜 프로퍼티, 프로토콜 메서드

//프로포톨 프로포티 : 연산프로퍼티로 쓰든 저장 프로퍼티로 쓰든 상관하지 않는다.
//명세하지 않기에, 구현을 할 때 프로퍼티를 저장 프로퍼티로 쓸 수도 있고 연산 프로퍼티로 사용할 수도 있다.
//무조건 var로 선언해야 한다.
//만약에 get만 명시했다면, get기능만 최소한으로 구현되어 있으면 된다! 그래서 필요하다면 set도 구현해도 괜찮다.
...
@objc protocol AnnieTableVieProtocol {
