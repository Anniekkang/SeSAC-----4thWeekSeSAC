...
class LocationViewController: UIViewController {
    ...
    @IBAction func notificationButtonClickced(_ sender: UIButton) {
    ...
    //Notificaion 3.권한 허용한 사용자에게 알림 요청 (언제? 어떤콘텐츠?)
    //iOS 시스템에서 알림을 담당 > 알림등록
    
    /*
     - 권한 허용 해야만 알림이 온다
     - 권한 허용 문구 시스템적으로 최초 한 번만 뜬다.
     - 허용 X 인 경우 애플 설정으로 직접 유도하는 코드를 구성 해야 한다.
     - 기본적으로 알림은 포그라운드에서 수신되지 않는다.
     - 로컬 알림에서 60초 이상 반복 가능 / 갯수제한 64개 / 커스텀 사운드 30초
     
     
     
     
     1. 뱃지제거? > 언제 제거하는게 맞을까?
     2. 노티 제거? > 노티의 유효 기간은? 카톡 vs 잔디 > 언제 삭제하는게 맞을까?
     3. 포그라운드 수신? > 딜리게이트 메서드로 해결!
     
     +a
     - 노티드 앱 실행이 기본인데, 특정 노티를 클릭할 때 특정 화면으로 가고 싶다면?
     ...
     */
        ...
        func sendNotification(){
            ...
            //언제 보낼 것인가? 1. 시간간격, 2. 캘린더 3. 위치에 따라 설정가능.
